...
enum ImGuiFileBrowserFlags_
{
    ...
    ImGuiFileBrowserFlags_Window = 1 << 10,
};
...
namespace ImGui
{
    ...
    public:
        ...
        // true when ImGuiFileBrowserFlags_MultipleSelection is enabled, only
        // one of selected filename will be returned
    ...
    private:
        ...
        std::filesystem::path           draggedPath_;
        unsigned int rangeSelectionStart_; // enable range selection when shift
                                           // is pressed

        std::vector<FileRecord> fileRecords_;

        // IMPROVE: truncate when selectedFilename_.length() >
...
inline bool ImGui::FileBrowser::IsHovered() const noexcept
{
    return isHovered_;
}

inline bool ImGui::FileBrowser::IsFocused() const noexcept
{
    return isFocused_;
}
...
inline void ImGui::FileBrowser::Display()
{
    ...
    if (flags_ & ImGuiFileBrowserFlags_Window)
    {
        if (!Begin(openLabel_.c_str(),
                   nullptr,
    ...
    else if (flags_ & ImGuiFileBrowserFlags_NoModal)
    {
    ...
    isFocused_       = ImGui::IsItemFocused();
    isOpened_        = true;
    bool       isWnd = (flags_ & ImGuiFileBrowserFlags_Window);
    ScopeGuard endPopup(
        [&isWnd]
        {
            if (isWnd)
                           ...
                           ImGuiSelectableFlags_DontClosePopups))
            {
                ...
                if (rangeSelect)
                {
                    const unsigned int first =
                        (std::min)(rangeSelectionStart_, rscIndex);
                    const unsigned int last =
                        (std::max)(rangeSelectionStart_, rscIndex);
                    ...
                    for (unsigned int i = first; i <= last; ++i)
                    {
                        ...
                        if (!wantDir &&
                            !IsExtensionMatched(fileRecords_[i].extension))
                        {
                            continue;
                        }
