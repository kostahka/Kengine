...
//  ImGuiPlatformIO interface. 2023-11-13: Updated for recent SDL3 API changes.
...
static void ImGui_ImplSDL3_SetPlatformImeData(ImGuiViewport*        viewport,
                                              ImGuiPlatformImeData* data)
{
    ImGui_ImplSDL3_Data* bd     = ImGui_ImplSDL3_GetBackendData();
    SDL_Window*          window = (SDL_Window*)viewport->PlatformHandle;
    if ((data->WantVisible == false || bd->Window != window) &&
        bd->Window != NULL)
    {
...
static void ImGui_ImplSDL3_UpdateMouseData()
{
    ...
    if (io.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport)
    {
        ...
        if (SDL_Window* sdl_mouse_window =
                SDL_GetWindowFromID(bd->MouseWindowID))
 ...
 #define MAP_BUTTON(KEY_NO, BUTTON_NO)                                         \
     {                                                                         \
         io.AddKeyEvent(KEY_NO,                                                \
                        SDL_GetGamepadButton(gamepad, BUTTON_NO) != 0);        \
     }
...
void ImGui_ImplSDL3_NewFrame()
{
    ...
    // would be more rygorous, but testing for payload reduces noise and
...
static void ImGui_ImplSDL3_CreateWindow(ImGuiViewport* viewport)
{
    ...
    SDL_SetWindowPosition(
        vd->Window, (int)viewport->Pos.x, (int)viewport->Pos.y);
    vd->WindowOwned = true;
    if (use_opengl)
    {
                                          ...
                                          ImU64*         out_vk_surface)
{
    ...
    (void)vk_allocator;
    SDL_bool ret =
        SDL_Vulkan_CreateSurface(vd->Window,
